...
Установщик дополнительно предлагает обновления текущего выпуска ОС, выполняет продвинутую настройку сервера, защищает API ноды и по выбору устанавливает ЧебурNET Traffic Control. Удаление системных пакетов автоматически не выполняется.
...
| 05 | Продвинутая настройка без отдельных установок пакетов; разрешение TCP/443 даже при ранее активном UFW; вывод правил и проверка ограничений API |
...
Traffic Control внутри этой сборки имеет базовую версию `1.0.0` и отдельную редакцию интеграции `Vision 1.1.4`. Она отличается от самостоятельного релиза TC исправлениями диагностики, списков, ACME и интерфейса. Самостоятельные репозитории компонентов этим изменением не обновляются
