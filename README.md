...
- Remnawave Node в Docker из тега `remnawave/node:latest`; после загрузки образ закрепляется по дайджесту SHA-256;
...
| 04 | Загрузка Remnawave Node `latest`, закрепление по дайджесту и запуск nginx через два Unix-сокета |
...
Docker-ключ загружается с официального HTTPS-адреса, ограничен `Signed-By` и сверяется с отпечатком Docker `9DC8 5822 9FC7 DD38 854A E2D8 8D81 803C 0EBC FD88`. При новой установке загружается тег `remnawave/node:latest`, затем полученный дайджест атомарно сохраняется в Compose. При `--resume` уже закреплённый образ сохраняется. Это доверие к содержимому реестра на момент загрузки, а не заранее проверенный образ; совместимость новых версий требует проверки. Работающие серверы автоматически не обновляются. Встроенные Auto Tuning и Traffic Control закреплены SHA-256 на этапе сборки.
