...
| 00 | Согласие на индекс APT; отдельные планы зависимостей и `full-upgrade`, повторная проверка перед применением; остановка при удалениях |
...
Разрешение запрашивается на обновление индекса APT и отдельно на каждый непустой план зависимостей, `full-upgrade`, Docker и nginx. Показываются новые, обновляемые и удаляемые пакеты, а также версии. Если план изменился перед применением, требуется новое согласие. Планы с удалениями останавливают установку для ручного разбора; фактический APT запускается с `--no-remove`. Phased updates Ubuntu не форсируются. Встроенный Auto Tuning имеет собственную логику установки недостающих пакетов безопасности в рамках согласия на установку; его код в 1.1.2 не менялся.
