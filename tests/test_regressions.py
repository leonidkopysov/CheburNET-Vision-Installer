...
class AuditTests(unittest.TestCase):
    def test_os_fields_are_optional_and_do_not_leak(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(p.stdout, 'debian::|original|1.1.2')
    ...
    def test_version_without_root(self):
        ...
        self.assertEqual(p.stdout.strip(), '1.1.2')
...
''', {'FAIL': fail})
    ...
    def test_resume_preserves_pending_exit_code(self):
        with tempfile.TemporaryDirectory() as td:
            base = Path(td)/'node'; base.mkdir(); (base/'.cheburnet-managed').write_text('1.1.2\n')
